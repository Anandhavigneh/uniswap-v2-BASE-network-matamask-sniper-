...
def init_web3():
    config = load_config()
    primary_rpc = config["base_rpc"]
    fallback_rpc = config.get("fallback_rpc", None)
    web3 = Web3(Web3.HTTPProvider(primary_rpc))

    # Try primary RPC
    try:
        if web3.is_connected():
            logger.info(f"System Event - Connected to Primary RPC: {primary_rpc}")
            return web3
        else:
            logger.error(f"System Event - Primary RPC connection failed: {primary_rpc}")
    except Exception as e:
        logger.error(f"System Event - Primary RPC connection error: {e}")

    # Try fallback RPC if available
    if fallback_rpc:
        logger.info(f"System Event - Attempting Fallback RPC: {fallback_rpc}")
        web3 = Web3(Web3.HTTPProvider(fallback_rpc))
        try:
            if web3.is_connected():
                logger.info(f"System Event - Connected to Fallback RPC: {fallback_rpc}")
                return web3
            else:
                logger.error(f"System Event - Fallback RPC connection failed: {fallback_rpc}")
        except Exception as e:
            logger.error(f"System Event - Fallback RPC connection error: {e}")

    logger.error("System Event - All RPC connections failed. Exiting.")
    exit(1)
...
async def monitor_trade():
    ...
    while active_trade:
        try:
            ...
            if TSL_ENABLED and "max_profit" in active_trade:
                trigger_pct = active_trade["max_profit"] - TRAILING_PCT
                if profit_pct <= trigger_pct:
                    logger.info(f"Trade Condition - TSL triggered for {active_trade['pair']}, Peak: {active_trade['max_profit']:.1f}%, Current: {profit_pct:.1f}%")
                    ...
                    if tx_hash:
                        ...
                        await bot.send_message(
                            chat_id=config["allowed_chat_id"],
                            text=f"🛑 Trade closed (TSL triggered)! Sold at {profit_pct:.2f}% profit. Peak: {active_trade['max_profit']:.1f}%. TX: https://basescan.org/tx/{tx_hash.hex()}"
                        )
                        trade_data = {
                            "type": "sell",
            ...
            if profit_pct >= config["take_profit_percent"]:
                ...
                if tx_hash:
                    ...
                    await bot.send_message(
                        chat_id=config["allowed_chat_id"],
                        text=f"🎯 Trade closed (Take-profit triggered)! Sold at {profit_pct:.2f}% profit. TX: https://basescan.org/tx/{tx_hash.hex()}"
                    )
                break
            ...
            if loss_pct >= config["stop_loss_percent"]:
                ...
                if tx_hash:
                    ...
                    await bot.send_message(
                        chat_id=config["allowed_chat_id"],
                        text=f"🛑 Trade closed (Stop-loss triggered)! Sold at {loss_pct:.2f}% loss. TX: https://basescan.org/tx/{tx_hash.hex()}"
                    )
                    trade_data = {
                        "type": "sell",
                        "pair": active_trade["pair"],
                        ...
                        if ((symbol_pair == pair or symbol_pair == reverse_pair) or
                            ...
                            if liquidity >= min_liq:
                                ...
                                if active_trade is None:
                                    ...
                                    logger.warning(f"Trade Initiation - Trade already active for {active_trade['pair']}")
                            else:
...
async def force_close_trade(reason="manual"):
    global active_trade, monitor_task, stop_monitoring, web3
    if not active_trade:
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.error(f"Trade Closure - Attempt {attempt + 1}/{max_retries} failed: {e}")
            if attempt < max_retries - 1:
                logger.info(f"Trade Closure - Retrying after {retry_delay} seconds...")
                await asyncio.sleep(retry_delay)
                web3 = init_web3()
            else:
                logger.error(f"Trade Closure - Max retries reached: {e}")
                await bot.send_message(
                    chat_id=config["allowed_chat_id"],
                    text=f"❌ Emergency sell error after retries: {str(e)}"
                )
                active_trade = None
                return None
...
async def cmd_setting(message: Message):
    ...
    logger.info(f"System Event - Displayed settings: {msg.replace('\n', '; ')}")
...
async def cmd_trade(message: Message):
    ...
    logger.info(f"System Event - Active trade info: {msg.replace('\n', '; ')}")
    await message.reply(msg)
...
async def cmd_balance(message: Message):
    ...
    logger.info(f"System Event - Wallet balances: {msg.replace('\n', '; ')}")
    await message.reply(msg)
...
async def cmd_help(message: Message):
    help_msg = (
        ...
        "/balance - Show blockchain wallet balances\n"
...
async def main():
    logger.info("System Event - Bot initializing with config")
...
if __name__ == "__main__":
    logger.info("System Event - Bot started")
    asyncio.run(main())